...
const stockPrices = {
  AAPL: { price: 185.5, change: 2.4, volume: 52000000, marketCap: "2.9T" },
  TSLA: { price: 245.2, change: 5.7, volume: 41000000, marketCap: "778B" },
  NVDA: { price: 425.8, change: 3.2, volume: 35000000, marketCap: "1.05T" },
  GOOGL: { price: 125.3, change: -1.2, volume: 28000000, marketCap: "1.57T" },
  AMZN: { price: 142.75, change: 1.8, volume: 33000000, marketCap: "1.48T" },
  MSFT: { price: 365.2, change: 0.9, volume: 25000000, marketCap: "2.71T" },
...
const generatePortfolioOptimization = (
  riskLevel,
  investmentAmount,
  preferences = {},
) => {
  ...
  const portfolio = stocks.map((symbol) => ({
    ...
    amount: (
      (allocation.stocks / stocks.length) *
...
const generateChatResponse = (message, userId = null) => {
  const responses = {
    ...
    diversify:
      "I recommend diversifying across sectors: 30% tech (AAPL, NVDA), 25% healthcare (JNJ, PFE), 20% finance (JPM, BAC), 15% consumer goods (PG, KO), 10% bonds/cash for stability.",
    "expected return":
      "With your current allocation (60% stocks, 30% bonds, 10% cash), expected annual return is 8-12%. Tech heavy weighting increases potential but adds volatility.",
  ...
  try {
    ...
    if (url === "/api/portfolio/optimize" && method === "POST") {
      ...
      if (!riskLevel || !amount) {
        return res
          .status(400)
          .json({ error: "Risk level and amount required" });
      }

      if (amount < 100) {
        return res
          .status(400)
          .json({ error: "Minimum investment amount is $100" });
      }
